...
export default function UniversityFooter() {
  const { brandLead, brandTrail, tagline, description, columns, copyright, note } =
    university.footer;
        ...
        <div className="grid gap-12 lg:grid-cols-[2fr_1fr_1fr_1fr]">
          ...
          {/* <div className="max-w-sm">
            <div className="flex items-center gap-2.5">
              <FiSearch className="h-6 w-6 shrink-0 text-brand" />
              <span className="font-display text-2xl font-semibold tracking-tight">
                <span className="text-brand">{brandLead}</span>{" "}
                <span className="text-ink">{brandTrail}</span>
              </span>
            </div>
            <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-[#9AA6BC]">
              {tagline}
            </p>
            <p className="mt-5 text-sm leading-relaxed text-[#5b6c84]">
              {description}
            </p>
          </div> */}
